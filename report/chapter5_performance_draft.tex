...
factorisation and the triangular solves. Throughout the chapter the
per-step cost of a configuration is the sum of its per-phase medians over
the steps, the cost of a typical step, robust against the turbo and
...
and 64 elements per connection: nonzeros of the computed factors and
representative per-step cost over 100 semi-implicit time steps. The reported
cost is obtained as the sum of the median times measured for matrix rebuild,
factorisation and linear solve within the same benchmark run, making the
measurement less sensitive to fluctuations. The reported timings are
therefore intended for comparisons within this ordering study and should not
be compared directly with the wall-clock times reported in
Table~\ref{tab:seqbaseline}, which were obtained in a different benchmark
session. RCM is applied as an explicit symmetric permutation of the matrix;
the other variants reorder internally.}
\label{tab:ordering}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../benchmarks/28_sequential_ordering_study/results/performance_ordering.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../benchmarks/29_static_condensation/results/performance_condensation.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../benchmarks/32_hybrid_condensation/results/performance_scaling.pdf}
