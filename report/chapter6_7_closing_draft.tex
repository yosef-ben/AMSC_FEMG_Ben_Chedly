...
This project set out to bring the finite element method to partial
differential equations posed on metric graphs and to exercise the resulting
library on a problem of current interest, the Fisher--Kolmogorov model of
neurodegeneration on the brain connectome. The library rests on a small
core: a graph topology with metric connections, a piecewise linear
discretisation along every connection and a common workflow of coefficient
evaluation, assembly, solution and export, from which the concrete problems
of the report derive. The benchmarks of Section 3 validated this core on
cases with known solutions: stationary and time-dependent problems on the
star graph, with the expected first-order accuracy in time of Backward
Euler and second-order accuracy of Crank--Nicolson, eigenvalue problems on
the star, graphene-like and binary tree graphs against their analytical
spectra and the comparison between the metric-graph operator and the
combinatorial graph Laplacian.

The study of Section 4 used the library as a laboratory for the
Fisher--Kolmogorov equation on the 83-region connectome of [9]. Three
findings stand out. First, at one element per connection the metric-graph
formulation reproduces the behaviour of the nodal network models at no
additional cost, while refinement inside the connections remains available
whenever sharper fronts require it. Second, the balance between reaction
and transport, summarised by the Damköhler number, governs both the
...
regionally heterogeneous reaction rates of [8] transferred to our
connectome, the lobes cross the staging thresholds in the clinical order,
which suggests a division of roles between the transport pathways and the
regional reaction rates.

The performance analysis of Section 5 closed the loop between the
mathematics and the implementation. The cost anomaly of the sequential
baseline was traced to the fill-in of the default ordering; the numbering
of the unknowns, interiors first and vertices last, was recognised as an
arrow structure whose exact elimination is the static condensation of the
step matrix; the condensed step parallelises naturally over the
connections. On a four-core laptop the per-step cost of the finest
connectome discretisation falls by a combined factor of about thirty with
respect to the best full-system factorisation, and every intermediate
version was validated step by step against the sequential solver at the
level of round-off.

Several directions remain open.
\begin{itemize}
\item The MPI version was measured on a single node, where the threaded
version is preferable; its purpose is distributed memory and a multi-node
...
aggregation.
\item Richer disease models from the literature, with several protein
species or patient-specific parameters, can be hosted on the same
discretisation layer without changes to its structure.
\end{itemize}

Every quantitative claim of this report is backed by a stored and
regenerable record, as described in the next section; we regard this
reproducibility, together with the validated equivalence of every
optimisation, as a result of the project in its own right.
...
Python 3 with NumPy and Matplotlib. The default build type is Release with
\texttt{-O2}.
