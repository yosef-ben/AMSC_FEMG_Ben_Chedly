...
an unlabelled oblique view, the graph inside a translucent brain surface,
uniform silver vertices, and straight connections whose colour and thickness
both grow with the fibre number of the connection, under a rainbow bar
...
choice had to be made here: both scale linearly between the smallest and the
largest fibre number. The distribution is strongly skewed, so most
connections sit at the thin blue bottom of the ramp and a handful of short
association bundles carry the warm colours; the printed figure shows the
same aspect, which is what made the linear choice the comparable one.
...
PANELS = (
    ("sagittal_right", "sagittal", "left"),
    ("coronal", "coronal", "right"),
    ("longitudinal", "longitudinal", "left"),
    ("oblique", None, None),
)
NODE_SILVER = 0.80
...
def main():
    ...
    norm = matplotlib.colors.Normalize(fibres.min(), fibres.max())
    ...
    with tempfile.TemporaryDirectory() as scratch:
        ...
        for view, _, _ in PANELS:
            panels[view] = render(
                ...
                edge_radii=radii, scale=scale, surface_opacity=0.08)[0]

        box = common_box(list(panels.values()))
        figure, axes = plt.subplots(2, 2, figsize=(8.6, 7.0))
        for axis, (view, label, side) in zip(axes.flat, PANELS):
    ...
    # no numbers. It sits under the oblique view.
    bar_axis = figure.add_axes([0.615, 0.055, 0.21, 0.020])
    ...
    print(f"  fibre number range: [{fibres.min():g}, {fibres.max():g}], "
          f"colour and width follow the linear ramp")
