...
| `27/seeding_patterns_regional` (report: the tau seeding with the regional rates of Corti et al., rescaled to the same mean, `--only tau`) and `27/seeding_patterns_regional_full` (record: both rows) | `plot-connectome-staging.py --prefix seeding_patterns_regional` | `staging/{tau,amyloid}_regional/fem_profiles.csv`, plus the reference strips | `test_fisher_kolmogorov_fornari83 ... be_lumped <seed> benchmarks/21/results/reaction_coefficients.csv` |
...
| `27/regional_biomarker_curves` (report: the four lobe biomarkers of the regional tau run, below the render rows) | `plot-regional-biomarker-curves.py` | `tau_regional_biomarkers.csv` | `test_fisher_kolmogorov_fornari83`, `be_lumped`, regional rates |
