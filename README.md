...
Every report figure is generated by a script from a file written by one of the
executables. `benchmarks/FIGURES.md` records the chain for each of them, and

```bash
/usr/bin/python3 scripts/verify-figures.py
```

re-derives from the stored results the quantities the figures and the READMEs
state, failing if any of them disagree.
...
The three connectome figures also need `vtk`, which renders the anatomical
panels offscreen:

```bash
/usr/bin/python3 -m pip install vtk
```
